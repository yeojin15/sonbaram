...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
            ...
            plugin.each(function (i, wrapper) {
                ...
                if (plugin.settings.preloaded.length) {
                    ...
                    for (let i = 0; i < plugin.settings.preloaded.length; i++) {
                        $uploadedContainer.append(createImg(plugin.settings.preloaded[i].src, plugin.settings.preloaded[i].id, plugin.settings.preloaded[i].info, plugin.settings.preloaded[i].oid, true));
                    }
        ...
        let createImg = function (src, id, info, oid, preloaded) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                // Create the img tag
                // $text = $('<textarea name="'+plugin.settings.extendsTextareaName+'[]" placeholder="20자 내외로 작성하세요" class="text-image">', {class: 'text-image'}).html(info).appendTo($container),
                $text_id = $('<input type="hidden" name="oid[]" value="'+(oid!=""?oid:null)+'" />').appendTo($container),

                // Create the delete button
                $button = $('<button>', {class: 'delete-image'}).appendTo($container),

                // Create the delete icon
                $i = $('<i>', {class: 'fal fa-times'}).appendTo($button);
